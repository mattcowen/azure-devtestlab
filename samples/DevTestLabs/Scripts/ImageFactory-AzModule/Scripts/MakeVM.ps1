...
$existingVms = Get-AzResource -ResourceType "Microsoft.DevTestLab/labs/virtualMachines" -ODataQuery "substringof(Name, '$DevTestLabName')" | Where-Object { $_.Name -eq "$DevTestLabName/$vmName"}
...
if($existingVms.Count -ne 0){
    ...
    if($vmDeployResult.ProvisioningState -eq "Succeeded"){
        ...
        $existingVm = Get-AzResource -ResourceType "Microsoft.DevTestLab/labs/virtualMachines" -ODataQuery "substringof(Name, '$DevTestLabName')" | Where-Object { $_.Name -eq "$DevTestLabName/$vmName"}

        #Determine if artifacts succeeded
        $filter = '$expand=Properties($expand=ComputeVm,NetworkInterface,Artifacts)'
        $vmResource = Get-AzureRmResource -ResourceType 'Microsoft.DevTestLab/labs/virtualmachines' -Name $existingVm.Name -ResourceGroupName $existingVm.ResourceGroupName -ExpandProperties -ODataQuery $filter
